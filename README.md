...
2. Deploy RAN
   * **No-ACK HARQ-ACK**: ```source experiments.sh nr``` 
   * **ACK-NACK HARQ-ACK**: ```source experiments.sh nr harq```
   * **NACK-Only HARQ-ACK**: Modify the BWP allocation on the [configuration file](https://github.com/AlexStolt/OpenAirInterface5G-Multicast-Broadcast-Services-MBS/blob/main/conf/gnb-nr-harq.conf) and then ```source experiments.sh nr harq```
