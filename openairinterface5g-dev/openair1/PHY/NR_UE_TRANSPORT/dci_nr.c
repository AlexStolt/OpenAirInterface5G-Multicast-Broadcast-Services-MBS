...
// AlexStolt: The UE will get the MBMS RNTI when it receives it from the DCI on nr_dci_decoding_procedure(). 
// This will be used in MBMS nr_dlsch_unscrambling() as the RNTI to avoid LDPC 
// errors when decoding PHY signals with the mbms_rnti (instead of the RNTI of the DLSCH).
int32_t mbms_rnti = -1;
  ...
  for (int j = 0; j < rel15->number_of_candidates; j++) {
    ...
    for (int k = 0; k < rel15->num_dci_options; k++) {
      ...
      nr_pdcch_unscrambling(&pdcch_e_rx[e_rx_cand_idx],
                            rel15->coreset.scrambling_rnti, // AlexStolt: This is always 0 according to my observations with one UE
                            L * 108,
                            rel15->coreset.pdcch_dmrs_scrambling_id,
                            tmp_e);

      uint32_t crc = polar_decoder_int16(tmp_e, dci_estimation, 1, NR_POLAR_DCI_MESSAGE_TYPE, dci_length, L);
      ...
      if(crc == M_RNTI){
        ...
        // AlexStolt: The DCI contains the M_RNTI and not the actual RNTI. 
        // So we translate M_RNTI values to n_rnti to make the UE think that this is unicast.
        LOG_D(NR_PHY_DCI, "[AlexStolt] Setting MBMS RNTI %x to UE RNTI %x\n", crc, n_rnti);
        crc = (uint32_t) n_rnti;
      }
      ...
      if (crc == n_rnti) {
        ...
        // AlexStolt: Use DLSCH RNTI if the MBMS RNTI is not set or if the RNTI used is an SI_RNTI, 
        // else use the MBMS RNTI for unscrabling and avoiding LDPC errors down the line.
        if(mbms_dci){
