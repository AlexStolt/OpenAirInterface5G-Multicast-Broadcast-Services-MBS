  ...
  switch (pdu_type){
    ...
    case FAPI_NR_RX_PDU_TYPE_DLSCH:
      if(dlsch0) {
        ...
        LOG_D(PHY, "[AlexStolt] Called nr_fill_rx_indication with RNTI %x and dl_harq0->ack %u\n", dlsch0->rnti, dl_harq0->ack);
                                   ...
                                   int G)
{
  ...
  if(ret < ue->max_ldpc_iterations + 1){
    ...
    AssertFatal(true, "[AlexStolt] %s:%d dlsch decoding, ret = %d\n", __FILE__, __LINE__, ret);
  }
  ...
  switch(dlsch[0].rnti_type) {
    ...
    case TYPE_SI_RNTI_:
      LOG_D(PHY, "[AlexStolt] TYPE_SI_RNTI_:FAPI_NR_RX_PDU_TYPE_SIB\n");
      ind_type = FAPI_NR_RX_PDU_TYPE_SIB;
      break;

    case TYPE_C_RNTI_:
      ind_type = FAPI_NR_RX_PDU_TYPE_DLSCH;
      LOG_D(PHY, "[AlexStolt] TYPE_C_RNTI_:FAPI_NR_RX_PDU_TYPE_DLSCH\n");
      break;
  ...
  if(is_cw1_active) {
    ...
    if(ret1 > ue->max_ldpc_iterations){
      AssertFatal(true, "[AlexStolt] %s:%d dlsch decoding, ret1 = %d\n", __FILE__, __LINE__, ret1);
    }
  ...
  if (dlsch[0].rnti_type == TYPE_C_RNTI_) {
    ...
    if(dlsch[0].rnti != M_RNTI)
      LOG_D(PHY, "[AlexStolt] Unicast ACK/NACK for RNTI %x PID %d\n", dlsch[0].rnti, harq_pid);
    else
      LOG_D(PHY, "[AlexStolt] Multicast ACK/NACK for RNTI %x PID %d\n", dlsch[0].rnti, harq_pid);
...
int pbch_pdcch_processing(PHY_VARS_NR_UE *ue, const UE_nr_rxtx_proc_t *proc, nr_phy_data_t *phy_data)
{
  ...
  if ((frame_rx%64 == 0) && (nr_slot_rx==0)) {
    ...
    double retransmission_percentage = 0;
    for (int round = 1; round < 16 && (round < 3 || ue->dl_stats[round] != 0); ++round){
      p += snprintf(p, end - p,"/%d", ue->dl_stats[round]);
      retransmission_percentage = retransmission_percentage + ue->dl_stats[round];
    }
    retransmission_percentage = ((double) retransmission_percentage * 100) / ((double) ue->dl_stats[0]);
    LOG_I(NR_PHY,"%s Retransmission Percentage %.2f\n", output, retransmission_percentage);
