...
int get_rnti_type(const NR_UE_MAC_INST_t *mac, const uint16_t rnti)
{
  ...
  } else if (rnti == mac->crnti || rnti == M_RNTI) {
                                   ...
                                   fapi_nr_dci_indication_pdu_t *dci_ind)
{
  ...
  if (R > 0) {
    ...
    if (!dlsch_pdu->new_data_indicator && current_harq->TBS != dlsch_pdu->TBS) {
      LOG_W(NR_MAC, "DCI 10 NDI indicates re-transmission but computed TBS %d doesn't match with what previously stored %d\n", dlsch_pdu->TBS, current_harq->TBS);
                                   ...
                                   fapi_nr_dci_indication_pdu_t *dci_ind)
{
  ...
  dlsch_pdu->k1_feedback = feedback_ti;
  LOG_D(MAC, "[AlexStolt]: HARQ Feedback for PID %d K1 %d RI %d DAI %d\n", dci->harq_pid.val, dlsch_pdu->k1_feedback, dci->pucch_resource_indicator, dci->dai[0].val);
  ...
  if (R > 0) {
    ...
    // storing for possible retransmissions
    if (!dlsch_pdu->new_data_indicator && current_harq->TBS != dlsch_pdu->TBS) {
      LOG_W(NR_MAC, "DCI 11 NDI indicates re-transmission but computed TBS %d doesn't match with what previously stored %d\n", dlsch_pdu->TBS, current_harq->TBS);
      
      LOG_D(NR_MAC, "[AlexStolt] PID %d MCS %d R %d Qm %d RBs %d symbols %d nb_re_dmrs %d num_dmrs %d nb_rb_oh %d Nl %d NDI %d\n",
            dci->harq_pid.val,
            dlsch_pdu->mcs,
            R,
            dlsch_pdu->qamModOrder,
            dlsch_pdu->number_rbs,
            dlsch_pdu->number_symbols,
            nb_re_dmrs,
            dlsch_pdu->dlDmrsSymbPos,
            nb_rb_oh,
            Nl,
            dlsch_pdu->new_data_indicator
      );
