                                ...
                                const int32_t mbms_Y)
{
  ...
  for (int maxL = 16; maxL > 0; maxL >>= 1) {
    ...
    for (int j = 0; j < max_number_of_candidates; j++) {
      ...
      // AlexStolt: Search for MBMS DCI if MBMS is enabled
      if(mbms_Y != -1){
        int mbms_cce = aggregation * ((mbms_Y + ((j * N_cces) / (aggregation * max_number_of_candidates)) + 0) % (N_cces / aggregation));
        LOG_T(NR_MAC_DCI,
            "Candidate %d of %d mbms_cce %d (L %d N_cces %d Y %d)\n",
            j,
            max_number_of_candidates,
            mbms_cce,
            aggregation,
        ...
        duplicated = false;
        for (int k = 0; k < i; k++) {
          if (rel15->CCE[k] == mbms_cce && rel15->L[k] == aggregation) {
            duplicated = true;
            LOG_T(NR_MAC_DCI, "Candidate %d of %d is duplicated\n", j, max_number_of_candidates);
          }
        }
        if (!duplicated) {
          rel15->CCE[i] = mbms_cce;
          rel15->L[i] = aggregation;
          i++;
        }
      }
                    ...
                    const NR_SearchSpace_t *ss)
{
  ...
  if (ss->searchSpaceType->present == NR_SearchSpace__searchSpaceType_PR_ue_Specific){
    ...
    // CCE for M_RNTI
    mbms_Y = get_Y(ss, slot, M_RNTI);
  }
  fill_dci_search_candidates(ss, rel15, Y, mbms_Y);
