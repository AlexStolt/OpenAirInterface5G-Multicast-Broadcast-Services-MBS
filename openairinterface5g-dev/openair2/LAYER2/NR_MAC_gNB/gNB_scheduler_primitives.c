...
uint16_t *synched_ta_frame = NULL;
                      ...
                      frame_t frame)
{
  ...
  const int num_dl_sched = (int)(stats->rounds[0] - bler_stats->rounds[0]); // Transmission
  const int num_dl_retx = (int)(stats->rounds[1] - bler_stats->rounds[1]); // Retransmissions
...
// From all UEs find an HARQ PID to sync to
int find_common_harq_pid(NR_UE_sched_ctrl_t **list, int length, bool ndi_synchronization_enabled){  
  AssertFatal(list != NULL, "[AlexStolt] List is NULL...\n"); 
  AssertFatal(length != 0, "[AlexStolt] Length is 0...\n"); 
  AssertFatal(list[0] != NULL, "[AlexStolt] First List is NULL...\n");

  int current = list[0]->available_dl_harq.head;
  while(current != -1){
    bool found_in_all = true;
    int current_ndi = list[0]->harq_processes[current].ndi;

    for (int i = 1; i < length; i++){
      bool found_in_current_other_list = false;

      AssertFatal(list[i] != NULL, "[AlexStolt] List at %d is NULL...\n", i); 
      
      int current_other = list[i]->available_dl_harq.head;
      while(current_other != -1){
        int current_other_ndi = list[i]->harq_processes[current_other].ndi;
        if(ndi_synchronization_enabled){
          if(current_other == current && current_other_ndi == current_ndi){
            found_in_current_other_list = true;
            break;
          }
        }
        else {
          if(current_other == current){
            found_in_current_other_list = true;
            break;
          }
        }
        current_other = list[i]->available_dl_harq.next[current_other];
      }

      if(!found_in_current_other_list){
        found_in_all = false;
        break;
      }
    }
    ...
    current = list[0]->available_dl_harq.next[current];
  }

  return -1;
}

void dump_harq_list(NR_list_t *list, rnti_t rnti){
  char buffer[1024]; 
  int offset = 0;
  ...
  if (!list) {
    LOG_E(NR_MAC, "HARQ list is NULL for RNTI %d\n", rnti);
    return;
  } 

  while (current != -1) {
    offset += sprintf(buffer + offset, "  %d", current);
  ...
  LOG_E(NR_MAC, "HARQ LIST for RNTI %d is %s\n", rnti, buffer);
}
...
// AlexStolt: GDB never reached this call 
...
int nr_mac_enable_ue_rrc_processing_timer(gNB_MAC_INST *mac, NR_UE_info_t *UE, bool apply_cellgroup)
{
  ...
  if(!synched_ta_frame){
    ...
    synched_ta_frame = &UE->UE_sched_ctrl.ta_frame;
  }
  else {
    UE->UE_sched_ctrl.ta_frame = *synched_ta_frame;
    LOG_A(NR_MAC, "[AlexStolt] RNTI %x TA %d from %d\n", UE->rnti, UE->UE_sched_ctrl.ta_frame, (mac->frame - 1 + 1024) % 1024);
  }
