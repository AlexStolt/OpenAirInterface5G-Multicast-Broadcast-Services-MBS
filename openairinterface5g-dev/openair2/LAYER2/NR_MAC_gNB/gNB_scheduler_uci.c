                           ...
                           int harq_round_max)
{
  ...
  if (success) {
    LOG_D(NR_MAC, "[AlexStolt] Received ACK UCI for RNTI %x PID %d\n", UE->rnti, harq_pid);
    finish_nr_dl_harq(sched_ctrl, harq_pid);
  } else if (harq->round >= harq_round_max - 1) {
    LOG_D(PHY, "[AlexStolt] Abort UCI as HARQ for RNTI %x PID %d (%d/%d)\n", UE->rnti, harq_pid, harq->round, harq_round_max - 1);
    abort_nr_dl_harq(UE, harq_pid);
  } else {
    LOG_D(PHY,"[AlexStolt] Received NACK UCI for RNTI %x PID %d\n", UE->rnti, harq_pid);
                             ...
                             const nfapi_nr_uci_pucch_pdu_format_0_1_t *uci_01)
{
  ...
  if (((uci_01->pduBitmap >> 1) & 0x01)) {
    ...
    for (int harq_bit = 0; harq_bit < uci_01->harq.num_harq; harq_bit++) {
      ...
      NR_UE_harq_t *harq = find_harq(frame, slot, UE, nrmac->dl_bler.harq_round_max); // AlexStolt: Uplink UCI
      if (!harq) {
        LOG_D(NR_MAC, "UE %04x: Could not find a HARQ process at %4d.%2d!\n", UE->rnti, frame, slot);
        break;
      }
                          ...
                          int is_common)
{
  ...
  for (int f = 0; f < fb_size; f++) {
    ...
    // AlexStolt: Each MBMS UE must have the same TDD allocation for PUCCH 
    if(mbms_tunnel != NULL && UE->mbms_pucch_allocation != -1 && pucch_index != UE->mbms_pucch_allocation){
      LOG_D(NR_MAC, "Current PUCCH Index %d != MBMS PUCCH Index %d\n", pucch_index, UE->mbms_pucch_allocation);
      continue;
    }
    ...
    if (curr_pucch->active &&
      ...
      // AlexStolt: In case of MBMS the PUCCH index is common for every UE in the MBMS group
      LOG_D(NR_MAC, "RNTI %x DL %4d.%2d, UL_ACK %4d.%2d Scheduling ACK/NACK in PUCCH %d with timing indicator %d DAI %d\n",
            UE->rnti, frame, slot, curr_pucch->frame, curr_pucch->ul_slot, pucch_index, f, curr_pucch->dai_c);
