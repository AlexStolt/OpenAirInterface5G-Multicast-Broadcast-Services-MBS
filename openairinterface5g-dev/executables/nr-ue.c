...
static int UE_dl_preprocessing(PHY_VARS_NR_UE *UE, const UE_nr_rxtx_proc_t *proc, int *tx_wait_for_dlsch, nr_phy_data_t *phy_data)
{
    ...
    if(phy_data->dlsch[0].rnti != 0)
      LOG_D(PHY, "[AlexStolt] Called UE_dl_preprocessing with RNTI %x\n", phy_data->dlsch[0].rnti);
...
void UE_dl_processing(void *arg) {
  ...
  if(phy_data->dlsch[0].rnti != 0)
    LOG_D(PHY, "[AlexStolt] Called UE_dl_processing with RNTI %x\n", phy_data->dlsch[0].rnti);
